    ...
    try {
      ...
      if (sig.side === 'LONG 📈') {
        if (currentPrice >= sig.tp1 && !sig.tp1Hit) {
          sig.tp1Hit = true;
          await sendTelegram('@mrcrypto166',
...
async function isUserSubscribed(userId) {
  ...
  try {
    const url = `https://api.telegram.org/bot${BOT_TOKEN}/getChatMember?chat_id=@mrcrypto166&user_id=${userId}`;
    ...
    console.error('isUserSubscribed error:', e);
    return false;
  }
}
